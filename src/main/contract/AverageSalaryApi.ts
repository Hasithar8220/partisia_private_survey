...
export class AverageSalaryApi {
   ...
   */
  private readonly buildAddRatingRpc = (amount: number): Buffer => {
    // First build the public inputs
    const fnBuilder = new FnRpcBuilder("add_rating", this.abi);
    const additionalRpc = fnBuilder.getBytes();

    // Then build the secret input
    const secretInputBuilder = ZkInputBuilder.createZkInputBuilder("add_rating", this.abi);
